...
class MenuSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Menu
        fields = ["id", "restaurant", "menu_date", "menu_items", "menu_items_input"]
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            menu_items = [MenuItem(menu=menu, **item_data) for item_data in menu_items_data]
    ...
    def validate(self, data):
        if self.context['request'].method == 'POST':
            if 'menu_items_input' not in data or not data['menu_items_input']:
                raise serializers.ValidationError({"menu_items_input": "At least one menu item is required."})

            names = [item['name'] for item in data['menu_items_input']]

            if len(names) != len(set(names)):
                raise serializers.ValidationError({"menu_items_input": "Menu item names must be unique within a menu."})
        return data
