...
class TestVoteViews:
    ...
    def test_vote_results_old_version(
            self, client, user, restaurant, menu_item, get_jwt_token
    ):
        user.restaurant = restaurant
        user.save()
        Vote.objects.create(
            employee=user, menu_item=menu_item, vote_date=timezone.now().date()
        )
        token = get_jwt_token(user)
        client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {token}", HTTP_X_BUILD_VERSION="1.0"
        )
        response = client.get("/api/voting/results/")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1

    def test_vote_results_new_version(
            self, client, user, restaurant, menu_item, get_jwt_token
    ):
        user.restaurant = restaurant
        user.save()
        ...
        token = get_jwt_token(user)
        client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {token}", HTTP_X_BUILD_VERSION="2.0"
        )
        response = client.get("/api/voting/results/")
        assert response.status_code == status.HTTP_200_OK
